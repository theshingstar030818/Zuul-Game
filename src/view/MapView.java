...
public class MapView extends JFrame implements Observer {
	...
	public void update(Observable arg0, Object arg1) {
		if (arg1 instanceof Player) {
			...
			// Recursively find rooms to display beginning at the center of the
			// map
	...
	/**
	 * This method recursively travels through the game and builds a 2d array of
	 * size SIZExSIZE of the rooms for the MapView to display
	 * 
	 * @param x
	 * @param y
	 * @param rooms
	...
	private void discoverRooms(int x, int y, Room[][] rooms) {
		...
		if (rooms[x][y].getExit(NORTH) != null) { // If there is a north exit
			if (x > 0 && rooms[x - 1][y] == null) { // If x is within the bounds
													// of the array (x>0) and
		...
		if (rooms[x][y].getExit(SOUTH) != null) { // If there is a south exit
			if (x < (SIZE - 1) && rooms[x + 1][y] == null) { // If x is within
																// the bounds of
																// the array
																// (SIZE) and
																// the room to
		...
		if (rooms[x][y].getExit(WEST) != null) { // If there is a west exit
			if (y > 0 && rooms[x][y - 1] == null) { // If y is within the bounds
													// of the array (y>0) and
		...
		if (rooms[x][y].getExit(EAST) != null) { // If there is an east exit
			if (y < (SIZE - 1) && rooms[x][y + 1] == null) { // If Y is within
																// the bounds of
																// the array
																// (SIZE) and
																// the room to
